...
public final class NoFall extends MovementModule {
    ...
    protected void onMovementTick(MinecraftClient client) {
        if (client.player == null || client.player.fallDistance < minFallDistance.get()) {
            return;
        }

        if (mode.is("Motion")) {
            client.player.setVelocity(client.player.getVelocity().x, Math.max(client.player.getVelocity().y, -0.1), client.player.getVelocity().z);
