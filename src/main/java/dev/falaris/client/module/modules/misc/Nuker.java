...
public final class Nuker extends MiscModule {
    ...
    private final IntegerSetting delay = setting(new IntegerSetting("Delay", "Ticks between nuker passes.", 2, 1, 20));
    ...
    private final BooleanSetting creativeOnly = setting(new BooleanSetting("Creative Only", "Only run while creative.", true));
    ...
    protected void onMiscTick(MinecraftClient client) {
        ...
        for (BlockPos pos : BlockPos.iterateOutwards(origin, radius, radius, radius)) {
            if (broken >= blocksPerTick.get()) {
                return;
            }
            if (!allowed(client, pos)) {
                continue;
            }
    ...
    private boolean allowed(MinecraftClient client, BlockPos pos) {
        ...
        if (mode.is("Flatten") && pos.getY() > client.player.getBlockY() - 1) {
            return false;
        }
        if (mode.is("Forward")) {
            Vec3d toBlock = pos.toCenterPos().subtract(client.player.getEyePos()).normalize();
            if (client.player.getRotationVector().dotProduct(toBlock) < 0.82) {
                return false;
            }
        }
        BlockState state = client.world.getBlockState(pos);
        return !(ignoreAir.enabled() && (state.isAir() || !state.getFluidState().isEmpty())) && state.getHardness(client.world, pos) >= 0.0f;
    }
